...
#                fig.colorbar(refl_max,cax=cbar_ax2, label='Max Reflectivity (dBZ)', orientation='horizontal')
    ...
    general_outpath = '/home/disk/meso-home/crs326/Documents/Research/WRF_Upscale_Growth_Paper/Shear_SALLJ_DBZ_single_MCS_time/Figures_'
    
    specific_outpath = '%sarea_%s%s%s' %(MCS_file_label, MCS_init_area, SALLJ_search_text, env_search_text)
    
    plt.savefig(general_outpath + specific_outpath + '/%s_%s_init_area_WRF_%sZ_relaxed_SALLJ_strength_%d_shear_barbs_smooth_level_barbs_and_smooth_sfc_barbs_blue_few_%s_larger_dots.png' %(MCS_init_area, MCS_file_label, conditions_datetime_dt.strftime('%Y%m%d%H'), level, '1_panel'), dpi=600)
