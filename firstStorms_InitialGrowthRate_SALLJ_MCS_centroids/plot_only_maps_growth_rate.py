...
if events_removed == True:
    ...
    events_bool = np.full(len(MCS_growth_stage_time_length_filtered), True)
...
if seperation_plot == True:
    ...
    MCS_ccs_area_growth_rate_filtered_mask = MCS_ccs_area_growth_rate_filtered[mask]
    ...
    MCS_ccs_area_growth_rate_filtered_mask = MCS_ccs_area_growth_rate_filtered[:]
...
################ plot MCS init centroids colored by growth rate ####################

cc_growth_rate = [
    'blue' if (growth_rate >= 10000) else 'black'
    for growth_rate in MCS_ccs_area_growth_rate_filtered_mask
]

#    if(dir_shear_chosen_layer <= 45) or (dir_shear_chosen_layer >= 315):
#        cc = 'blue'
#    elif 135 <= dir_shear_chosen_layer <= 225:
#        cc = 'red'
#    else:
#        cc = 'black'
        
# plot MCS centroid
axs.scatter(MCS_center_lons_initiation_filtered_mask, MCS_center_lats_initiation_filtered_mask, transform=crs.PlateCarree(), color=cc_growth_rate, marker='*', zorder=7, s=1600)

############### plot growth rate for each MCS ####################

if plot_label == True:

    for lon, lat, growth_rate in zip(MCS_center_lons_initiation_filtered_mask, MCS_center_lats_initiation_filtered_mask, MCS_ccs_area_growth_rate_filtered_mask):
        plt.text(lon+0.15, lat, round(growth_rate), transform=crs.PlateCarree())
...
general_outpath = '/home/disk/meso-home/crs326/Documents/Research/WRF_Upscale_Growth_Paper/3SALLJ_MCS_centroids'

plt.savefig(general_outpath + '/5%s%s%s_initCentroids_%s_%s%s%s.png' %(MCS_file_label, offset_label, var_seperation_label, plot_var, MCS_init_area, label, events_removed_label), dpi=600)
