...
################ plot MCS init centroids colored by SALLJ coverage ####################

cc_area_SALLJ = [
    'blue' if (area_SALLJ >= .3) else 'black'
...
# plot MCS centroid
axs.scatter(MCS_center_lons_initiation_filtered_mask, MCS_center_lats_initiation_filtered_mask, transform=crs.PlateCarree(), color=cc_area_SALLJ, marker='*', zorder=7, s=1600)

############### plot SALLJ coverage for each MCS ####################

if plot_label == True:

    for lon, lat, area_SALLJ in zip(MCS_center_lons_initiation_filtered_mask, MCS_center_lats_initiation_filtered_mask, MCS_prop_area_SALLJ_mask):
        plt.text(lon+0.15, lat, round(area_SALLJ, 2), transform=crs.PlateCarree())
