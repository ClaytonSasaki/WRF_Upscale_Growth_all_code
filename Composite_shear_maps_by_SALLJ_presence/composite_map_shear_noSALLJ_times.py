...
Creates .dat files and plots of composite shear for all NON-SALLJ times
...
Uses '3relaxed_noSALLJ_times_VMRS_1101_0430.dat' file copied over. See README for more.
...
if Calculate == True:
    ...
    noSALLJ_time_list = pickle.load(open("3relaxed_noSALLJ_times_%s_%s_%s.dat" %(noSALLJ_location, start_date, end_date), "rb"))

    noSALLJ_time_list_arr = np.array([str(noSALLJ_time) for noSALLJ_time in noSALLJ_time_list])

    noSALLJ_time_list_arr_dt = [datetime.datetime.strptime(date, '%Y-%m-%d %H:%M:%S') for date in noSALLJ_time_list_arr]

    variable1_spatial_noSALLJ_times = np.empty((len(noSALLJ_time_list_arr_dt), 499, 599))
    variable1_spatial_noSALLJ_times[:] = np.NaN
    
    variable2_spatial_noSALLJ_times = np.empty((len(noSALLJ_time_list_arr_dt), 499, 599))
    variable2_spatial_noSALLJ_times[:] = np.NaN
    
    variable3_spatial_noSALLJ_times = np.empty((len(noSALLJ_time_list_arr_dt), 499, 599))
    variable3_spatial_noSALLJ_times[:] = np.NaN

    noSALLJ_count = 0

    plot_terrain = False

    for noSALLJ_time in noSALLJ_time_list_arr_dt:

        dt_date_str = noSALLJ_time.date().strftime('%Y%m%d')

        file_name = '/wrfout_d01_%s-%s-%s_%s:00:00' %(noSALLJ_time.strftime('%Y'), noSALLJ_time.strftime('%m'), noSALLJ_time.strftime('%d'), noSALLJ_time.strftime('%H'))
        ...
        print('reading in variables for: %s %s'  %(dt_date_str, noSALLJ_time.strftime('%H')))
        ...
        variable1_spatial_noSALLJ_times[noSALLJ_count,:,:] = mag_diff1
        variable2_spatial_noSALLJ_times[noSALLJ_count,:,:] = mag_diff2
        variable3_spatial_noSALLJ_times[noSALLJ_count,:,:] = mag_diff3

        #mean_variable_spatial_noSALLJ_times = np.nanmean(variable_spatial_noSALLJ_times, axis=0)

        noSALLJ_count = noSALLJ_count + 1

        #print(variable_spatial_noSALLJ_times[:,0,0])
        #print(mean_variable_spatial_noSALLJ_times)
        
        ncfile.close()

    mean_variable1_spatial_noSALLJ_times = np.nanmean(variable1_spatial_noSALLJ_times, axis=0)
    print(mean_variable1_spatial_noSALLJ_times)

    pickle.dump(mean_variable1_spatial_noSALLJ_times, open("mean_%s_noSALLJ_times_%s_%s.dat" %('0_6km_shear', start_date, end_date), "wb"))
    
    mean_variable2_spatial_noSALLJ_times = np.nanmean(variable2_spatial_noSALLJ_times, axis=0)
    print(mean_variable2_spatial_noSALLJ_times)

    pickle.dump(mean_variable2_spatial_noSALLJ_times, open("mean_%s_noSALLJ_times_%s_%s.dat" %('0_3km_shear', start_date, end_date), "wb"))
    
    mean_variable3_spatial_noSALLJ_times = np.nanmean(variable3_spatial_noSALLJ_times, axis=0)
    print(mean_variable3_spatial_noSALLJ_times)

    pickle.dump(mean_variable3_spatial_noSALLJ_times, open("mean_%s_noSALLJ_times_%s_%s.dat" %('2_6km_shear', start_date, end_date), "wb"))
...
if Plot == True:
    ...
    mean_variable_spatial_noSALLJ_times = pickle.load(open("mean_%s_noSALLJ_times_%s_%s.dat" %('2_6km_shear', start_date, end_date), "rb"))

    mean_variable_spatial_noSALLJ_times_smooth = smooth2d(mean_variable_spatial_noSALLJ_times, 6, cenweight=4)

    levels = np.arange(5,55,5)

    shear_0_6km = plt.contour(lons, lats, mean_variable_spatial_noSALLJ_times_smooth, levels=levels, cmap='Reds', transform=crs.PlateCarree())
    ...
    plt.savefig(outpath + 'shear_2_6km_composite_map_noSALLJ_times_%s_%s_%s.png' %(start_date, end_date, noSALLJ_location))
    ...
    #plt.savefig(outpath + 'shear_0_6km_map_%s_%s.png' %(dt_date_str, noSALLJ_time.strftime('%H')))
