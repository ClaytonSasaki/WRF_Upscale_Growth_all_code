...
for count, (MCS_datetime, MCS_center_lon, MCS_center_lat) in enumerate(zip(MCS_datetime_initiation_str_filtered, MCS_center_lons_initiation_filtered, MCS_center_lats_initiation_filtered)):
    ...
    hght_original = getvar(wrf_ncfile, 'height', msl=False, units='m')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])].values # in m NOTE: this is AGL not MSL!!
    u_original = getvar(wrf_ncfile, 'ua', units='kt')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])].values # in kts
    v_original = getvar(wrf_ncfile, 'va', units='kt')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])].values # in kts
    ...
    u3km = interplevel(u_original, hght_original, 2000).values
    v3km = interplevel(v_original, hght_original, 2000).values

    u3km = interplevel(u_original, hght_original, 3000).values
    v3km = interplevel(v_original, hght_original, 3000).values
    
    u3km = interplevel(u_original, hght_original, 6000).values
    v3km = interplevel(v_original, hght_original, 6000).values
    ...
    ###############
    #### NEED TO FIGURE OUT HOW BELOW WORKS WITH ARRAY INSTEAD OF ONE VALUE #######
    ################
...
general_outpath = '/home/disk/meso-home/crs326/Documents/Research/WRF_Upscale_Growth_Paper/3shear_direction_MCS_centroids'
