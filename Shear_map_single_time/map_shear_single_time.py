...
Plots maps of shear for a single time
...
Saves wind shear over various layers.
...
outpath = '/home/disk/meso-home/crs326/Documents/Research/WRF_Upscale_Growth_Paper/Shear_map_single_time/'

# Open the NetCDF file
path_wrf = '/home/disk/monsoon/relampago/raw/wrf/'

#change date to be plotted
plot_date = 2018120517

plot_date_dt = datetime.strptime(str(plot_date), '%Y%m%d%H')
...
if Calculate == True:
    ...
    variable1_spatial_time = np.empty((499, 599))
    variable1_spatial_time[:] = np.NaN
    
    variable2_spatial_time = np.empty((499, 599))
    variable2_spatial_time[:] = np.NaN
    
    variable3_spatial_time = np.empty((499, 599))
    variable3_spatial_time[:] = np.NaN

    plot_terrain = False

    file_name = '/wrfout_d01_%s-%s-%s_%s:00:00' %(plot_date_dt.strftime('%Y'), plot_date_dt.strftime('%m'), plot_date_dt.strftime('%d'), plot_date_dt.strftime('%H'))

    file_path = path_wrf + plot_date_dt.strftime('%Y%m%d') + file_name
    ...
    print('reading in variables for: %s'  %(plot_date_dt.strftime('%Y%m%d%H')))
    ...
    variable1_spatial_time[:,:] = mag_diff1
    variable2_spatial_time[:,:] = mag_diff2
    variable3_spatial_time[:,:] = mag_diff3
    ...
    pickle.dump(variable1_spatial_time, open("%s_%s.dat" %(plot_date_dt.strftime('%Y%m%d%H'), '0_6km_shear'), "wb"))
    
    pickle.dump(variable2_spatial_time, open("%s_%s.dat" %(plot_date_dt.strftime('%Y%m%d%H'), '0_3km_shear'), "wb"))
    
    pickle.dump(variable3_spatial_time, open("%s_%s.dat" %(plot_date_dt.strftime('%Y%m%d%H'), '2_6km_shear'), "wb"))
...
if Plot == True:
    ...
    variable1_spatial_time = pickle.load(open("%s_%s.dat" %(plot_date_dt.strftime('%Y%m%d%H'), '0_6km_shear'), "rb"))

    variable1_spatial_time_smooth = smooth2d(variable1_spatial_time, 6, cenweight=4)

    levels = np.arange(5,55,5)

    shear_0_6km = plt.contour(lons, lats, variable1_spatial_time_smooth, levels=levels, cmap='Reds', transform=crs.PlateCarree())
    ...
    plt.savefig(outpath + '%s_0_6km_shear_map.png' %(plot_date_dt.strftime('%Y%m%d%H')))
